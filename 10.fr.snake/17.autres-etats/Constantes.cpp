...
const int Constantes::ECRAN_HAUTEUR { 617 };
...
const int Constantes::CARTE_TAILLE_Y { 16 };
...
int Constantes::carteX(float x) {
  float xNormalise = x - ECRAN_LARGEUR / 2
    + CARTE_TAILLE_X / 2 * SEGMENT_TAILLE;
  return round(xNormalise) / (int)Constantes::SEGMENT_TAILLE;
}

int Constantes::carteY(float y) {
  float yNormalise = y - ECRAN_HAUTEUR / 2
    + CARTE_TAILLE_Y / 2 * SEGMENT_TAILLE;
  return round(yNormalise) / (int)Constantes::SEGMENT_TAILLE;
}